...
function renderVocabSetup(){const box=$("vocabSetup");if(!box)return;const rs=ranges();
 ...
 box.innerHTML=`<h2>単語撃退の準備</h2><p id="vocabAudioStatus" class="v-status"></p><div class="v-setup-grid">
  <label>範囲<select id="vocabRange"><option value="all">すべて</option>${rs.map(([r,n])=>`<option value="${esc(r)}">${esc(r)}（${n}語）</option>`).join("")}</select></label>
...
function vAsk(){const q=data.questions.find(x=>x.id===vs.queue[vs.index]);vs.started=performance.now();$("mascot").src="assets/sakiusa-idle.gif";
 ...
 if(vs.mode==="card"){
  ...
  $("vocabFlip").onclick=()=>{sakiAudio.play(q.audioNo);$("vocabBody").innerHTML=`<div class="v-flip-card">${backHtml(q)}</div><div class="v-self-buttons"><button data-self="correct" class="primary">○ 覚えていた</button><button data-self="wrong" class="secondary">× まだ</button><button data-self="later" class="secondary">△ 後で</button></div>`;
  ...
  document.querySelectorAll("#vocabBody .choice").forEach(b=>b.onclick=()=>{const elapsed=performance.now()-vs.started,good=b.dataset.answer===q.answer;
   ...
   $("vocabFeedback").innerHTML=`<div class="v-feedback ${good?"good":"bad"}"><h3>${good?"正解！":"惜しい！"}</h3>${backHtml(q)}</div><button id="vocabNext" class="primary">次へ</button>`;
...
function vFinish(early){const rs=vs.records,ok=rs.filter(r=>r.good).length,rate=rs.length?Math.round(ok/rs.length*100):0,
 ...
 $("vocabFinish").innerHTML=`<div class="center"><h2>${early?"途中結果":"単語撃退 完了！"}</h2><div class="big">${rate}%</div><p>今回 ${rs.length}問・正解 ${ok}問</p><h3>${msg}</h3></div><h3>間違えた単語</h3>${wrong.length?wrong.map(q=>`<div class="wrong-item"><b>${esc(q.prompt)} → ${esc(q.answer)}</b><p>${esc(q.sentence||"")}</p><p>${esc(q.comment||"")}</p></div>`).join(""):"<p>ありません。すばらしい！</p>"}<button id="vocabHome" class="primary">準備へもどる</button> ${wrong.length?'<button id="vocabRetryWrong" class="secondary">間違いだけもう一度</button>':""}`;
